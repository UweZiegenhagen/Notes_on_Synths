...
\node at (5,2) [box] (adsr1) {ADSR};
\node at (8,2) [box] (adsr2) {ADSR};
\node at (0.75,1) [box] (keyboard) {Keyboard};

\node at (0.25,4.8) (offset) {\footnotesize offset};
%\node [box,below=of offset] (belowoffset) {B};

\node at (10.25,4.5) (output) {output};
\node at (3,1.25) (gate) {gate};

\node at (0,3) (cv) {\footnotesize\parbox{\widthof{control}}{
    control \\ voltage }};
...
\draw [thick,->] (0.25,4.5) to [out=0,in=180] (vco2);

\draw [thick,->] (noise) -| (vcf);
\draw [thick,->] (vcf) to [out=120,in=90] (vca);
